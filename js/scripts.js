  ...
  document.addEventListener("click", (event) => {
    ...
    //открыть модальное окно "Полный список услуг и цен"
      ...
      fetch("db/db.json")
        ...
        .then((data) => {
          delete sliderForRepair;
          delete sliderForRepairCarousel;

          const ulListLeft = pupupRepair.querySelector(".nav-list-popup-repair"),
          ulListRight = pupupRepair.querySelector(".popup-repair-types-content-table");
          ulListLeft.innerHTML = '';
          ulListRight.innerHTML = '';
          
          data.forEach((element)=>{
            if(element.date){
              pupupRepair.querySelector(".popup-repair-types-content__head-date .date").innerText = element.date;
            }
            if (element.title) {
              const newElementLeft = document.createElement("button");
              newElementLeft.classList.add("button_o", "popup-repair-types-nav__item");
              newElementLeft.textContent = element.title;

              ulListLeft.append(newElementLeft);

              const newElementRight1 = document.createElement("table");
              newElementRight1.classList.add("popup-repair-types-content-table__list");

              const newElementRight2 = document.createElement("tbody");
              newElementRight1.append(newElementRight2);
              ...
              element.priceList.forEach((el)=>{
                ...
                newElementRight2.append(newElementRight3);
              });

              ulListRight.append(newElementRight1);
